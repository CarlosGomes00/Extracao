...
```{r K-means}
set.seed(123)
logCPM_matrix <- logCPM_matrix[1:length(metadados$SEX)]
centers <- cut(logCPM_matrix, breaks = 4)
centers_factor <- factor(centers)
sex_factor <- factor(metadados$SEX)
...
```{r MDS plot}
mds <- cmdscale(dist(logCPM))

mds_df <- data.frame(X = mds[, 1], Y = mds[, 2], Sexo = sex_factor)

library(ggplot2)
ggplot(mds_df, aes(x = X, y = Y, color = Sexo)) +
  geom_point() +
  labs(title = "MDS Plot com Sexo") +
  theme_minimal()
```
