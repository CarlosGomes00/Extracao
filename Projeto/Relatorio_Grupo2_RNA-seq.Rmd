...
Após uma análise exploratória, concluimos que seria interessante tentar perceber se o sexo, tipo de amostras, o nº e fase dos tratamentos cumulativos e a sobrevivência dos pacientes estavam refletidos nos dados de expressão génica através da análise de expressão diferencial. 
...
Com o objetivo de indentificar possíveis padrões ou agrupamentos nos dados de expressão diferencial que nos permitam de alguma forma perver características das amostras ou dos pacientes, especificamente a resposta deste ao tratamento, começamos por fazer Multi-Dimensional Scaling (MDS). MDS é uma representação visual e simplificada dos dados e da similaridade ou distância entre estes, permitindo assim a análise da relação entre os níveis de expressão. 
...
A análise do MDS plot revelou possíveis padrões ou agrupamentos dos níveis de expressão génica, o que nos levou a tentar encontrar este agrupamento (cluster) através de análises não supervisionadas como o clustering hierárquico ou k-means.

# **Clustering de genes**

Dada a expressão diferencial analisada anteriormente e os resultados obtidos num artigo anterior[^1], optámos por verificar se há a formação de clusteres associados ao sexo e à sobrevivência dos paicentes.
...
Relativamente à utilização do modelos de Machine Learning, vamos começar por verificar o impacto de algumas características no tempo de sobrevida dos pacientes analisados. 

Então, iremos utilizar o dataset "merged_df", adicionando ainda algumas colunas de metadados que possam ter relevancia na análise!  
...
# Definir o modelo
modelo_dt = ctree(sobrevida_paciente ~ ., data = dados_treino)
...
Depois fizemos um plot da árvore de decisão relativa aos dados de teste. É possível observar que os valores omissos encontram-se também eles na árvore, no Node 4.

```{r Árvores de Decisão_Plot}
...
testDT = predict(modelo_dt, dados_teste)
...
modelo_nb = naiveBayes(sobrevida_paciente ~ ., data = dados_treino)

testeNB = predict(modelo_nb, dados_teste)

```

Após o teste, procedeu-se à análise dos seus resultados utilizando uma tabela e calculando a sua precisão.

```{r NB - Avaliação}
table(testeNB, dados_teste$sobrevida_paciente)

precisao_NB = mean(testeNB == dados_teste$sobrevida_paciente)

precisao_NB
```
Na tabela obtida, é possível verificar bastnates missmatches entre os valores previstos pelo modelo e os valores reais. Calculando a sua precisão, verificou-se uma precisão de 41%, bastante mais baixa que o modelo anterior, podendo significar uma aleatoriedade do modelo criado. Isso pode acontecer devido a um mau tratamento de dados, má escolha de classes, ou mau uso dos hiperparâmetros. De qualquer forma, também não é um resultado satisfatório para a previsão dos nossos dados.
