...
variaveis_interessantes = c(1:4, 6, 8:10, 19, 28, 29, 52, 70, 73, 76:78, 90:95)
...
## Opções para a DE:
dge.estDisp.out <- estimateDisp(dge, design)
...
metadados_slect = metadados[,c(8, 21)] # Sleciona o SAMPLE_SITE e o OS_STATUS.
merged_df <- merge(DADOS, metadados_slect, by = "row.names")
...
autoplot(pca_result, data = as.data.frame(merged_df_clean), colour = "OS_STATUS", size = 2, main = "OS_STATUS") +
  scale_color_manual(values = legenda_personalizada) # Cinza são os NAs. 

scatterplot3d(pca_data, color = as.integer(merged_df_clean$OS_STATUS), pch = 16,
