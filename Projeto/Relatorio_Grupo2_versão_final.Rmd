...
```{r Import dos packages, echo=FALSE, warning=FALSE}
...
Através da interpretação do MDS plot obtido, vemos que há semelhança nos níveis de expressão, algo já esperado dado os resultados da análise de expressão diferencial que não indicaram uma grande quantidade de genes diferencialmente expressos.
...
Após o MDS plot, os 50 genes com uma maior expressão diferencial foram filtrados e agrupados de acordo com a sua similaridade através de algoritmos de clustering hierárquico, para visualizar a possível existência e hierarquia de clusteres tendo em conta a expressão dos genes.

```{r - preparação para clustering - seleção de dados com alta variação de exp}
...
dist_top50 <- dist(genes_top50, method = 'euclidean')
hc = hclust(dist_top50, method = 'ward.D2')
plot(hc)
```

Através do clustering hierárquico é possível ver que são formados dois grandes clusteres, indicando que pode ser possível agrupar os genes em 2 clusteres, porém após pesquisa dos genes presentes nos clusteres nenhuma relação clara foi estabelecida. 

## Clustering de genes de acordo com o sexo dos pacientes

```{r clustering hierarquico - sexo}
cluster <- hclust(dist_matrix, method = "ward.D2")  # Ward's method for linkage

plot(cluster, main = "Clustering hierárquico tendo em conta o sexo dos pacientes")
...
Enquanto continuamos a ver os dendogramas com 2 grandes clusteres, a análise do heatmap não revela nenhum padrão de expressão génica entre homens e mulheres, não havendo o agrupamento de genes em dois clusteres associados ao sexo.
...
Da mesma forma, a análise do heatmap não revela padrões ou clusteres para a sobrevivência dos pacientes baseado na expressão génica.


```{r}
set.seed(123)
logCPM_matrix <- logCPM_matrix[1:length(metadados$OS_STATUS)]
centers <- cut(logCPM_matrix, breaks = 3)
centers_factor <- factor(centers)
live_factor <- factor(metadados$OS_STATUS)

levels(centers_factor) <- c("Cluster 1", "Cluster 2", "Cluster 3")

table <- table(centers_factor, live_factor)
print(table)
```

Em contraste, ao distribuir os genes em 3 diferentes clusteres tendo em conta o estado de sobrevivência dos pacientes, obtivémos resultados que sugerem que os genes nos diferentes clusters podem estar envolvidos em diferentes processos biológicos dado que há uma distribuição diferente dos genes pelos 3 clusteres que parece não estar relacionada com o estado de sobrevivência dos pacientes. É interessante destacar que há um nº mais elevado de genes no Cluster 2, o que pode indicar uma possível associação destes com algum processo biológico mais importante para a sobrevivência do que os genes nos outros clusters. 

## Clustering de genes de acordo com a classificação ELN 2017

```{r CLUSTERNG ELN 2017}
set.seed(123)
logCPM_matrix <- logCPM_matrix[1:length(metadados$ELN_2017)]
centers <- cut(logCPM_matrix, breaks = 6)
centers_factor <- factor(centers)
eln17_factor <- factor(metadados$ELN_2017)

levels(centers_factor) <- c("Cluster 1", "Cluster 2", "Cluster 3", "Cluster 4", "Cluster 5", "Cluster 6")

table <- table(centers_factor, eln17_factor)
print(table)
```

## Clustering de genes de acordo com a classificação ELN 2008

```{r}
set.seed(123)
logCPM_matrix <- logCPM_matrix[1:length(metadados$ELN_2008)]
centers <- cut(logCPM_matrix, breaks = 5)
centers_factor <- factor(centers)
eln08_factor <- factor(metadados$ELN_2017)

levels(centers_factor) <- c("Cluster 1", "Cluster 2", "Cluster 3", "Cluster 4", "Cluster 5")

table <- table(centers_factor, eln08_factor)
print(table)
```

Para a análise do clustering de genes tendo em conta as classificações ELN de 2017 e 2008, os dados foram agrupados aleatóriamente num determinado nº de clusteres consoante as diferentes classificações usadas em cada ano (6 clusteres para a ELN 2017 e 5 para 2008). Os clusteres formados não aparentam ter um padrão de distribuição que possa sugerir que os dados de expressão génica possam ser associados e agrupados de acordo com as diferentes classificações.
...
merged_df <- merge(t(logCPM), metadados_select, by = "row.names")
...
Não foram encontradas associações entre a expressão de determinados genes e o sexo, sobrevivência ou classificação de pacientes consoante as regras ELN de 2017 e 2008.
